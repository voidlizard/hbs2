...
(define (nbind a b)   (join := --bind (join :: a b)))
(define (nbind-ro a b)   (join := --bind-ro (join :: a b)))

(define (nspawn-args extra)
  (begin '[]
...
(define (run-shell)
  (begin
    ...
    (run:proc:attached sudo (cons systemd-nspawn [nspawn-args *nspawn-extra-args]))
